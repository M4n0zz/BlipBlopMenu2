...
31 f3 26 0a 74 26 40 36 02 01  
04 00 11 80 ff 21 11 bb cd b5  
00 0e 48 11 15 df cd b5 00 26  
00 74 21 5e d3 7e fe 0b 20 09  
36 76 04 21 d1 5b cd d6 35 fb  
cd 15 df c3 7a 00 21 6f d3 11  
a7 d6 7a be 28 1a 01 5e d3 0a  
fe 76 20 09 fa 58 d3 a7 20 0c  
3e 0b 02 3a 12 1b 7e 12 3e 6a  
22 72 21 ea d6 cb 46 28 04 7d  
ea 3c d1 cb 4e 28 03 cd 30 d4  
cb 56 28 03 cd 42 d4 0e 46 3e  
c3 c9 00 fa 30 d7 a7 c0 f0 b4  
cb 4f 3e 00 28 01 3c ea 38 cd  
c9 21 00 d7 af be 20 01 34 fa  
5e d3 fe 1c c8 f0 b4 cb 47 c8  
fa c5 cf fe 07 d0 a7 c8 cd 27  
0d 18 f3 cd a8 d6 e2 f0 b4 cb  
57 c8 f0 b8 f5 cd 2d df 21 27  
df e5 cd e8 29 f1 e0 b8 c3 07  
23 cd 19 37 21 91 cf 36 94 af  
ea 94 cf cd 51 09 0e 1f 3e 9d  
cd a1 23 21 c4 70 cd 22 39 3e  
1c cd e6 35 06 03 21 53 ba 11  
e8 c6 e5 d5 c5 d5 cd 0f 79 51  
c8 a1 c8 98 c8 ad c8 fa 5e d3  
c3 bc 12 06 02 21 ea d6 7e a8  
77 c9 06 01 cd 9a c8 a0 c0 af  
ea 3c d1 c9 06 04 18 e9  
...
Part 2 - Map Script payload ($d66a-$d6a7)
...
Part 3 - OAM DMA payloads ($d6a8-$d6e9)
...
d66a-d6a5: MSP payload
d6a6-d6a7: MSP backup address
d6a8-d6e9: OAM DMA payloads
d6ea: OAM DMA payload flags
...
call $00b5			; CopyData - due to previous copyData, de points correctly
...
call $00b5			; CopyData - the stored version of .OAMDMApayload. 

; copy timos loader to df15
ld c, $48			; 72 bytes
ld de, $df15		; to the top of the stack
call $00b5			; CopyData - due to previous copyData, de points towards TimOS loader payload
...
.curmsp	
jp $007a			; a safe address to be replaced automatically by msp manipulator
...
ld de, $d6a7		; Original MSP backup address+1
...
bit 1, [hl]
jr z, .payload3 	
call $d430			; trespassing script

; if scriptstate's bit2=0, skip parkour script
.payload3
bit 2, [hl]
jr z, .endoam 
call $d442			; Motorbike script
...
;;;;;;;;;;;; OAM hijack payload ;;;;;;;;;;;;
call $d6a8			; if initial payload changes, change address acordingly
